...
def train(
    ...
    for step in range(n_steps):
        ...
        next_state_tensor = (
            torch.tensor(np.array([next_state]), dtype=torch.float).to(device)
            if not done else None
        )

        memory.push(state, action, next_state_tensor, torch.tensor([reward], device=device))
...
def optimize_model(
    ...
    loss = F.smooth_l1_loss(state_action_values, target_state_action_values.unsqueeze(1))
...
def select_model_action(
    ...
    board_flat = state.cpu().numpy().reshape(3, 3, 3)[:, :, 0].flatten()
    legal_actions = np.where(board_flat == 1)[0]  # empty positions only

    if random.random() > eps:
        with torch.no_grad():
            q_values = model(state).cpu().detach().numpy().flatten()
            q_values[board_flat != 1] = -np.inf  # mask illegal actions
